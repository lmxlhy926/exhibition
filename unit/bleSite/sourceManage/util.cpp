...
void util::updateWhiteDeviceList() {
    LOG_INFO << "start to updateWhiteDeviceList....";
    string ip;
    int port;
    if(getSynergyInfo(ip, port)){
        ...
        if(httpUtil::sitePostRequest(ip, port, request, response)){
            if(response.getInt("code") == 0){
                LOG_PURPLE << "==>updateWhiteDeviceList successfully, " << response.getString("msg") << ".......";
            }else{
                LOG_RED << "==>updateWhiteDeviceList failed, " << response.getString("msg") << ".......";
            }
        }else{
            LOG_RED << "==>updateWhiteDeviceList failed, cant access synergy site.......";
        }
    }else{
        LOG_RED << "==>updateWhiteDeviceList failed, cannot get synergy site.......";
    }
}
