...
struct CoordPointType{
    double x;           //x坐标
    double y;           //y坐标
    string device_id;   //雷达id
    uint identity;      //人员编号
};

struct ControlPointType{
    string device_id;       //雷达id
    string roomNo;          //房间
    uint identity;          //人员编号
    int ctrlIndex = -1;    //受控编号
...
private:
    ...
    ControlPointType latestControlPoint;    
public:
    //单参构造函数
    ...
    stripLight(const stripLight& sl){
        ...
        this->latestControlPoint = sl.latestControlPoint;
    }
    ...
    //更新物理灯带属性
    void updatePhysicalStrip(StripParamType strip);
...
private:
    ...
    */
    ...
    //打印开启关闭索引
    void printIndex(const std::vector<int> index2Open);
    ...
    //获取修正后的控制点位
    struct ControlPointType getRealControlPoint(struct ControlPointType controlPoint);
};
